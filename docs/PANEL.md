...
The photographic hero is the only Overview presentation of input voltage, output voltage/frequency, load and battery charge. The repeated metric row and the repeated `Состояние` summary are not rendered. The operating title and connection plaque sit on a light header surface; the photograph begins below them as a rounded inset scene, accompanied by the locked cyan corner decoration. A compact reserve-readiness plaque sits below the hero, followed by a battery-detail card containing actual bank voltage, confirmed battery count, charger temperature and vendor remaining time converted from minutes to hours/minutes. `Резерв готов` is reserved for Line Mode with at least 95% charge; a lower charge is explicitly `Резерв неполный`. On the target phone the photographic scene is 300 CSS pixels high: the UPS remains enlarged and grounded on the floor, while the battery-capacity plaque stays in the clear space above the cabinet. The startup placeholder uses the same geometry, avoiding a layout jump when the registry becomes available. The input and load plaques retain their cabinet alignment, the battery-count label stays on one line, and a 16px bottom inset keeps the complete battery card above the fixed Bottom Tab Bar. Decorative flow connectors are not rendered. Battery charge current is not displayed because the current backend does not expose a verified charge-current entity.

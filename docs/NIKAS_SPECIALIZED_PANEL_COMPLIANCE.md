...
**Audited production path:** `panel.py` → `stark-solarpower-panel-bundle.js?v=0.9.3` → `stark-solarpower-panel`
**Scope:** implemented in UI v0.9.3; phone field acceptance remains required
...
| Source-aware title return | PASS | Final navigation layer mounts one semantic 44px+ center button, keeps the second line exactly `UI v0.9.3`, requires and validates the route/timestamp pair once, rejects future/stale hand-offs and performs explicit HA navigation without `history.back()`. |
...
| Production delivery | PASS | `panel.py` uses one cache-busted autonomous bundle URL, UI version `0.9.3` and cache headers for its versioned static files. |

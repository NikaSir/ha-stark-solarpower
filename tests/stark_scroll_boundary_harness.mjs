...
const path = join(
  ...
  "stark_solarpower",
  "frontend",
  "stark-solarpower-panel-v091.js",
);
const source = readFileSync(path, "utf8").replace(/^import[^\n]+\n/, "");
const context = { customElements: { get: () => null } };
runInNewContext(`${source}\n;globalThis.guardApi = {
  shouldBlockNikasShellBoundaryMove,
  createNikasShellScrollBoundaryGuard,
};`, context);

const { shouldBlockNikasShellBoundaryMove, createNikasShellScrollBoundaryGuard } = context.guardApi;
...
assert.equal(move({ deltaY: -10 }), false, "top-edge upward movement must scroll natively");
...
assert.equal(move({ scrollTop: 500 }), false, "bottom-edge downward movement must scroll natively");
...
assert.equal(listeners.get("touchstart").options.passive, false, "touch sequence must remain cancelable on iOS");
...
assert.equal(prevented, false, "tap-sized finger jitter must not be canceled");
listeners.get("touchmove").listener({
  cancelable: true,
...
console.log("Stark Shell v2.1 scroll-boundary harness passed");
